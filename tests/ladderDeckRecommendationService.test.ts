...
describe("LadderDeckRecommendationService", () => {
  it("fetches validated data and writes a reusable cache", async () => {
    ...
    const result = await new LadderDeckRecommendationService({ sourceUrl: "https://example.com/cn.json", cachePath, fetcher, installationDetector: detected }).get("standard");
  ...
  it("states the capability boundary when no verified Chinese-server feed is configured", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "ladder-decks-")); dirs.push(root);
    const result = await new LadderDeckRecommendationService({ cachePath: path.join(root, "cache.json"), installationDetector: detected }).get("standard");
    expect(result).toMatchObject({ status: "unavailable" });
    expect(result.message).toContain("国服");
  });
  ...
  it("rejects a cache belonging to another patch", async () => {
    ...
    const result = await new LadderDeckRecommendationService({ cachePath, installationDetector: detected }).get("standard");
  ...
  it("keeps standard and wild caches side by side", async () => {
    ...
    const offline = new LadderDeckRecommendationService({ cachePath, installationDetector: detected });
