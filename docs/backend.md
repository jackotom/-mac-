...
Board-attack totals remain in tracker state. New users start with both board-attack overlay switches disabled; the monitor starts only after the user explicitly enables either switch. `QA_OPEN_BOARD_ATTACK_OVERLAY=1` still opens the deterministic full-display layer for automated rendering acceptance.
...
Arena screen choices use exact names first, then a unique best OCR match. Long names allow a bounded proportional edit distance because legendary-team frames commonly misread several Chinese characters and stylized digits at once. Ambiguous matches still fail closed. When several card IDs share one display name, the rated collectible/base card is preferred, so special variants such as `TOY_330t12` can reuse the real `TOY_330` statistics instead of producing an empty result.
