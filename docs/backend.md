...
`TAG_SCRIPT_DATA_NUM_1` has more than one meaning. The parser keeps its existing entity-link event and also publishes the numeric script-data event. For `REV_514` and `CORE_REV_514`, `TrackerEngine` records that number by controller as the current resurrection count, resolves abbreviated updates through the already known entity, accepts both inline `TAG_CHANGE` and `FULL_ENTITY` continuation forms, overwrites duplicate GameState/PowerTaskList copies, and clears the value at game boundaries.
